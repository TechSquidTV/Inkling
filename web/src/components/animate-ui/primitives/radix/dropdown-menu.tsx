 ...
 *
 * This file provides the low-level animated dropdown menu components that power
 * the high-level dropdown menu API. It integrates Radix UI primitives with our
 * highlight animation system.
 *
 * Architecture:
 * - Uses Radix UI for accessibility and behavior
 * - Integrates motion/react for smooth animations
 * - Wraps items with highlight tracking for hover effects
 * - Manages dropdown state via React context
 *
 * Note: DropdownMenuCheckboxItem uses React.forwardRef to enable ref forwarding
 * when used with the highlight system's asChild prop.
 */
'use client'
...
function DropdownMenuSubTrigger({
  ...
  const [, ,] = useDataState<HTMLDivElement>(
    'highlighted',
    highlightedRef,
    (value) => {
      if (value === true) {
...
>(function DropdownMenuCheckboxItem(
  { checked, onCheckedChange, disabled, onSelect, textValue, ...props },
  forwardedRef
) {
  const { setHighlightedValue } = useDropdownMenu()
  const highlightedRef = React.useRef<HTMLDivElement>(null)
